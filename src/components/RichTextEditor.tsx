  ...
  return (
    <div className='border rounded-md overflow-hidden'>
      ...
      <div className='border-b border-gray-300 p-2 flex gap-1 flex-wrap'>
        <ToolbarButton onClick={() => execCommand('bold')} title='Bold' isActive={activeStates.bold}>
          <strong>B</strong>
        </ToolbarButton>
        <ToolbarButton onClick={() => execCommand('italic')} title='Italic' isActive={activeStates.italic}>
          <em>I</em>
        </ToolbarButton>
        <ToolbarButton onClick={() => execCommand('underline')} title='Underline' isActive={activeStates.underline}>
          <u>U</u>
        </ToolbarButton>

        <div className='w-px bg-gray-300 mx-1' />

        <ToolbarButton onClick={insertCodeBlock} title='Code Block' isActive={activeStates.code}>
          <code className='px-1 rounded text-sm'>&lt;/&gt;</code>
        </ToolbarButton>

        <div className='w-px bg-gray-300 mx-1' />

        <ToolbarButton onClick={() => insertList(false)} title='Bullet List' isActive={activeStates.unorderedList}>
          <PiListBullets style={{ width: '30px', height: '30px' }} />
        </ToolbarButton>
        <ToolbarButton onClick={() => insertList(true)} title='Numbered List' isActive={activeStates.orderedList}>
      ...
      <div
        ...
        className={`rich-editor-content min-h-[300px] p-4 focus:outline-none ${
          isFocused ? 'ring-2 ring-blue-500' : 'focus:ring-2 focus:ring-blue-500'
        }`}
        style={{
