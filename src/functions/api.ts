...
export type RequestType = 'get' | 'post' | 'put' | 'patch' | 'delete' | 'postFormData'
...
let globalAuthTokenGetter: (() => Promise<string | null> | string | null) | null = null

export function setGlobalAuthTokenGetter(getter: () => Promise<string | null> | string | null) {
  globalAuthTokenGetter = getter
}

function buildAxiosConfig(method: Method, url: string, data: unknown, options: ApiOptions): AxiosRequestConfig {
  ...
  const config: AxiosRequestConfig = {
    method,
    url,
    baseURL: options.baseURL ?? process.env.NEXT_PUBLIC_API_BASE_URL ?? undefined,
    data,
...
export async function apiRequest<T = unknown>(
  ...
  const method: Method = type === 'postFormData' ? 'post' : (type as unknown as Method)
...
export const api = {
  get: <T>(url: string, options?: ApiOptions) => apiRequest<T>('get', url, undefined, options),
  post: <T>(url: string, body?: unknown, options?: ApiOptions) => apiRequest<T>('post', url, body, options),
  put: <T>(url: string, body?: unknown, options?: ApiOptions) => apiRequest<T>('put', url, body, options),
  patch: <T>(url: string, body?: unknown, options?: ApiOptions) => apiRequest<T>('patch', url, body, options),
  delete: <T>(url: string, body?: unknown, options?: ApiOptions) => apiRequest<T>('delete', url, body, options),
