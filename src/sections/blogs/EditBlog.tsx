...
const EditBlog = () => {
  ...
  if (loading) return <div className='flex justify-center items-center h-96 text-gray-500'>Loading blog details...</div>
      ...
      <section className='relative w-full h-56 sm:h-64 md:h-72 lg:h-80 overflow-hidden rounded-none sm:rounded-md mb-8'>
        ...
        <div className='absolute inset-0 flex items-center justify-center px-6'>
          <div className='text-center'>
            <h1 className='text-white text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tight'>Edit your Blog</h1>
            <p className='mt-3 text-white/90 text-sm sm:text-base max-w-2xl mx-auto'>Update your blog details below.</p>
          </div>
        </div>
      </section>
        ...
        <form onSubmit={handleSubmit} className='space-y-6'>
              ...
              {!image && !details.image ? (
                ...
                <div className='relative'>
                  <div className='relative w-full h-64 sm:h-96 rounded-md overflow-hidden border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800'>
                    <Image src={image || details.image} alt='Preview' fill className='object-contain' />
                  </div>
                  <button
                    type='button'
                    onClick={removeImage}
                    className='cursor-pointer absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full hover:bg-red-600 transition-colors shadow-lg'
                    title='Remove image'
                  >
          ...
          <div className='flex justify-end gap-4'>
            <button
              type='button'
              onClick={() => router.back()}
              className='cursor-pointer px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors'
            >
              Cancel
            </button>
            <button
              ...
              className='cursor-pointer px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2'
            >
